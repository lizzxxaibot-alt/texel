...
| T-009 | 2026-09-14 | `texel-funnel` | **Drop 1's 72-hour window has closed and its result is unwritten, while drop 2 is scheduled for the same run that must write it.** `funnel/LOG.md` commits in writing to reading the window at **2026-09-14 10:00** and the routine's next run is **2026-09-18** — four days late for its own date — at which point Phase 1 also wants drop 2. The numbers are already fixed and cannot be adjusted after the fact: `LEDGER.md` 09-11 = **46** Texel views (read ~3 h before the drop went up) and 09-14 = **63**, so **+17 over a clean 72 h interval**, against the drop's own pre-registered bar of *"materially more than ~30 views over the 72 h"*. The cheatsheet itself is **23 views / 11 downloads**, downloads **flat for 24 h**. `texel-watch` records those numbers and does **not** own the verdict. | `funnel/LOG.md`'s drop-1 row is filled from the **09-11 and 09-14 `LEDGER.md` rows** (not from a live reading taken on 09-18, which would be a 7-day window wearing a 72-hour label), with an explicit pass/fail against the success-vs-theatre test the drop wrote for itself — **and that is written before drop 2 ships**, per this file's clear-your-rows-first rule |
